...
2. **Testing**: 
   This package uses the `testthat` framework for testing. Tests are located in `tests/testthat/`. Any new feature or bug fix must be accompanied by appropriate tests. Run tests via `make test` or `devtools::test()` to verify changes.
