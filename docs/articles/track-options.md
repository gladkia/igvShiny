...
`futile.logger``::`[`flog.threshold`](https://rdrr.io/pkg/futile.logger/man/flog.threshold.html)`(``futile.logger``::`[`WARN`](https://rdrr.io/pkg/futile.logger/man/logger.options.html)`)`
...
`    tbl ``=`` ``tbl``,`\
`    trackConfig ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``displayMode ``=`` ``"EXPANDED"``, maxRows ``=`` ``20``)`\
`    ``)`\
`#> removeTracksByName`\
...
`#>   elementID: igv`\
`#>   trackName: my regions`\
`#>   bedFilepath: tracks/file1d3458162d6f.bed`\
`#>   color: `\
`#>   trackHeight: 50`\
...
`    ``)`\
\
`widget`` ``<-`` `[`igvShiny`](https://gladkia.github.io/igvShiny/reference/igvShiny.md)`(`\
`    ``genomeOptions``,`\
...
`#> 'tracks': onClick`\
\
[`names`](https://rdrr.io/r/base/names.html)`(``widget``$``x``$``tracks``[[``1``]``]``)`\
`#> [1] "name" "type" "url"`
...
\
`coverage`` ``<-`` `[`data.frame`](https://rdrr.io/r/base/data.frame.html)`(`\
`    chrom ``=`` ``"chr1"``,`\
`    start ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``7432000``, ``7437000``)``,`\
`    end ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``7436000``, ``7442000``)``,`\
`    value ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``0.2``, ``0.9``)`\
`    ``)`\
\
...
`    autoscale ``=`` ``TRUE``,`\
`    trackConfig ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`\
`        graphType ``=`` ``"points"``,`\
...
\
`gff3.url`` ``<-`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(`\
...
`    gff3URL ``=`` ``gff3.url``, indexURL ``=`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(``gff3.url``, ``".tbi"``)``,`\
`    colorByAttribute ``=`` ``"biotype"``,`\
`    displayMode ``=`` ``"EXPANDED"``,`\
`    visibilityWindow ``=`` ``1000000``,`\
`    colorTable ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`\
...
\
`bam.url`` ``<-`` ``"https://1000genomes.s3.amazonaws.com/phase3/data/HG02450/alignment/HG02450.mapped.ILLUMINA.bwa.ACB.low_coverage.20120522.bam"`\
\
[`loadBamTrackFromURL`](https://gladkia.github.io/igvShiny/reference/loadBamTrackFromURL.md)`(`\
`    ``session``, id ``=`` ``"igv"``, trackName ``=`` ``"reads"``,`\
`    bamURL ``=`` ``bam.url``, indexURL ``=`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(``bam.url``, ``".bai"``)``,`\
`    trackConfig ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`\
`        sort ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`\
`            option ``=`` ``"TAG"``, tag ``=`` ``"HP"``,`\
...
`#>   elementID: igv`\
`#>   trackName: reads`\
`#>   bam: https://1000genomes.s3.amazonaws.com/phase3/data/HG02450/alignment/HG02450.mapped.ILLUMINA.bwa.ACB.low_coverage.20120522.bam`\
...
\
`gwas`` ``<-`` `[`data.frame`](https://rdrr.io/r/base/data.frame.html)`(`\
`    CHR ``=`` ``"chr1"``,`\
`    BP ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``1000``, ``2000``)``,`\
`    SNP ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"rs1"``, ``"rs2"``)``,`\
`    P ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``1e-8``, ``1e-5``)`\
`    ``)`\
\
[`loadGwasTrack`](https://gladkia.github.io/igvShiny/reference/loadGwasTrack.md)`(`\
`    ``session``, id ``=`` ``"igv"``, trackName ``=`` ``"gwas"``, tbl.gwas ``=`` ``gwas``,`\
`    trackConfig ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`\
`        columns ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``chromosome ``=`` ``12``, position ``=`` ``13``, value ``=`` ``28``)`\
`        ``)`\
`    ``)`\
`#> removeTracksByName`\
`#>   trackNames: gwas`\
...
`#>   elementID: igv`\
`#>   trackName: gwas`\
`#>   gwasDataFilepath: tracks/file1d341ea02f53.gwas`\
`#>   color: red`\
`#>   trackHeight: 200`\
`#>   autoscale: FALSE`\
`#>   min: 0`\
...
\
`junctions.url`` ``<-`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(`\
...
`    url ``=`` ``junctions.url``, indexURL ``=`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(``junctions.url``, ``".tbi"``)``,`\
`    trackHeight ``=`` ``150``,`\
`    trackConfig ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`\
`        colorBy ``=`` ``"motif"``,`\
...
`    ``)`\
\
`startupTrack`` ``<-`` `[`list`](https://rdrr.io/r/base/list.html)`(`\
`    name ``=`` ``"genes"``,`\
`    type ``=`` ``"annotation"``,`\
`    format ``=`` ``"gff3"``,`\
`    url ``=`` ``gff3.url``,`\
...
`    ``)`\
\
`widget`` ``<-`` `[`igvShiny`](https://gladkia.github.io/igvShiny/reference/igvShiny.md)`(``genomeOptions``, tracks ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``startupTrack``)``)`\
[`str`](https://rdrr.io/r/utils/str.html)`(``widget``$``x``$``tracks``[[``1``]``]``)`\
`#> List of 6`\
`#>  $ name       : chr "genes"`\
`#>  $ type       : chr "annotation"`\
`#>  $ format     : chr "gff3"`\
...
`    `[`igvShiny`](https://gladkia.github.io/igvShiny/reference/igvShiny.md)`(``genomeOptions``, tracks ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``startupTrack``)``)`\
`    ``}``)`
