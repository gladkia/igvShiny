...
[`library`](https://rdrr.io/r/base/library.html)`(`[`igvShiny`](https://github.com/gladkia/igvShiny)`)`` ``futile.logger``::`[`flog.threshold`](https://rdrr.io/pkg/futile.logger/man/flog.threshold.html)`(``futile.logger``::`[`WARN`](https://rdrr.io/pkg/futile.logger/man/logger.options.html)`)`
...
`tbl`` ``<-`` `[`data.frame`](https://rdrr.io/r/base/data.frame.html)`(`` `` chrom ``=`` ``"chr1"``,`` `` start ``=`` ``7432000``,`` `` end ``=`` ``7436000`` `` ``)`` `` `[`loadBedTrack`](https://gladkia.github.io/igvShiny/reference/loadBedTrack.md)`(`` `` ``session``,`` `` id ``=`` ``"igv"``,`` `` trackName ``=`` ``"my regions"``,`` `` tbl ``=`` ``tbl``,`` `` trackConfig ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``displayMode ``=`` ``"EXPANDED"``, maxRows ``=`` ``20``)`` `` ``)`` ``#> removeTracksByName`` ``#> trackNames: my regions`` ``#> elementID: igv`` ``#> loadBedTrackFromFile`` ``#> elementID: igv`` ``#> trackName: my regions`` ``#> bedFilepath: tracks/file1ce43d1ea1b1.bed`` ``#> color: `` ``#> trackHeight: 50`` ``#> displayMode: EXPANDED`` ``#> maxRows: 20`
...
`genomeOptions`` ``<-`` `[`parseAndValidateGenomeSpec`](https://gladkia.github.io/igvShiny/reference/parseAndValidateGenomeSpec.md)`(`` `` genomeName ``=`` ``"hg38"``,`` `` initialLocus ``=`` ``"NDUFS2"`` `` ``)`` `` ``widget`` ``<-`` `[`igvShiny`](https://gladkia.github.io/igvShiny/reference/igvShiny.md)`(`` `` ``genomeOptions``,`` `` tracks ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` name ``=`` ``"genes"``,`` `` type ``=`` ``"annotation"``,`` `` url ``=`` ``"https://example.org/genes.bed"``,`` `` onClick ``=`` ``"alert('hi')"`` `` ``)`` `` ``)`` `` ``)`` ``#> Warning in FUN(X[[i]], ...): Ignoring invalid or unsupported track options in`` ``#> 'tracks': onClick`` `` `[`names`](https://rdrr.io/r/base/names.html)`(``widget``$``x``$``tracks``[[``1``]``]``)`` ``#> [1] "name" "type" "url"`
...
`coverage`` ``<-`` `[`data.frame`](https://rdrr.io/r/base/data.frame.html)`(`` `` chrom ``=`` ``"chr1"``,`` `` start ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``7432000``, ``7437000``)``,`` `` end ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``7436000``, ``7442000``)``,`` `` value ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``0.2``, ``0.9``)`` `` ``)`` `` `[`loadBedGraphTrack`](https://gladkia.github.io/igvShiny/reference/loadGenomeAnnotationTrack.md)`(`` `` ``session``, id ``=`` ``"igv"``, trackName ``=`` ``"coverage"``, tbl ``=`` ``coverage``,`` `` autoscale ``=`` ``TRUE``,`` `` trackConfig ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` graphType ``=`` ``"points"``,`` `` autoscaleGroup ``=`` ``"sampleA"``,`` `` logScale ``=`` ``FALSE`` `` ``)`` `` ``)`` ``#> removeTracksByName`` ``#> trackNames: coverage`` ``#> elementID: igv`` ``#> Warning in .sanitizeAndMergeOptions(base.msg.to.igv, trackConfig):`` ``#> User-provided trackConfig options conflict with function arguments and will be`` ``#> ignored: autoscaleGroup`` ``#> loadBedGraphTrack`` ``#> elementID: igv`` ``#> trackName: coverage`` ``#> tbl: [{"chr":"chr1","start":7432000,"end":7436000,"value":0.2},{"chr":"chr1","start":7437000,"end":7442000,"value":0.9}]`` ``#> color: gray`` ``#> trackHeight: 30`` ``#> autoscale: TRUE`` ``#> min: NA`` ``#> max: NA`` ``#> autoscaleGroup: -1`` ``#> graphType: points`` ``#> logScale: FALSE`
...
`gff3.url`` ``<-`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(`` `` ``"https://s3.amazonaws.com/igv.org.genomes/hg38/"``,`` `` ``"Homo_sapiens.GRCh38.94.chr.gff3.gz"`` `` ``)`` `` `[`loadGFF3TrackFromURL`](https://gladkia.github.io/igvShiny/reference/loadGFF3TrackFromURL.md)`(`` `` ``session``, id ``=`` ``"igv"``, trackName ``=`` ``"genes"``,`` `` gff3URL ``=`` ``gff3.url``, indexURL ``=`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(``gff3.url``, ``".tbi"``)``,`` `` colorByAttribute ``=`` ``"biotype"``,`` `` displayMode ``=`` ``"EXPANDED"``,`` `` visibilityWindow ``=`` ``1000000``,`` `` colorTable ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` protein_coding ``=`` ``"darkgreen"``,`` `` processed_transcript ``=`` ``"blue"``,`` `` default ``=`` ``"black"`` `` ``)`` `` ``)`` ``#> removeTracksByName`` ``#> trackNames: genes`` ``#> elementID: igv`` ``#> loadGFF3TrackFromURL`` ``#> elementID: igv`` ``#> trackName: genes`` ``#> dataURL: https://s3.amazonaws.com/igv.org.genomes/hg38/Homo_sapiens.GRCh38.94.chr.gff3.gz`` ``#> indexURL: https://s3.amazonaws.com/igv.org.genomes/hg38/Homo_sapiens.GRCh38.94.chr.gff3.gz.tbi`` ``#> color: gray`` ``#> colorTable: darkgreen,blue,black`` ``#> colorByAttribute: biotype`` ``#> displayMode: EXPANDED`` ``#> trackHeight: 50`` ``#> visibilityWindow: 1e+06`
...
| `coverageTrackHeight` | numeric | height of the coverage track in pixels (default 50) |
| `showCoverage` | logical | show coverage histogram above alignments (default `TRUE`) |
| `showAlignments` | logical | show alignment reads (default `TRUE`) |
| `alignmentRowHeight` | numeric | row height in pixels for `"EXPANDED"` mode (default 14) |
| `squishedRowHeight` | numeric | row height in pixels for `"SQUISHED"` mode (default 3) |
...
`bam.url`` ``<-`` ``"https://1000genomes.s3.amazonaws.com/phase3/data/HG02450/alignment/HG02450.mapped.ILLUMINA.bwa.ACB.low_coverage.20120522.bam"`` `` `[`loadBamTrackFromURL`](https://gladkia.github.io/igvShiny/reference/loadBamTrackFromURL.md)`(`` `` ``session``, id ``=`` ``"igv"``, trackName ``=`` ``"reads"``,`` `` bamURL ``=`` ``bam.url``, indexURL ``=`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(``bam.url``, ``".bai"``)``,`` `` trackConfig ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` sort ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` option ``=`` ``"TAG"``, tag ``=`` ``"HP"``,`` `` chr ``=`` ``"chr8"``, position ``=`` ``128750000`` `` ``)`` `` ``)`` `` ``)`` ``#> removeTracksByName`` ``#> trackNames: reads`` ``#> elementID: igv`` ``#> loadBamTrackFromURL`` ``#> elementID: igv`` ``#> trackName: reads`` ``#> bam: https://1000genomes.s3.amazonaws.com/phase3/data/HG02450/alignment/HG02450.mapped.ILLUMINA.bwa.ACB.low_coverage.20120522.bam`` ``#> index: https://1000genomes.s3.amazonaws.com/phase3/data/HG02450/alignment/HG02450.mapped.ILLUMINA.bwa.ACB.low_coverage.20120522.bam.bai`` ``#> displayMode: EXPANDED`` ``#> showAllBases: FALSE`` ``#> sort: TAG,HP,chr8,128750000`
...
`gwas`` ``<-`` `[`data.frame`](https://rdrr.io/r/base/data.frame.html)`(`` `` CHR ``=`` ``"chr1"``,`` `` BP ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``1000``, ``2000``)``,`` `` SNP ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"rs1"``, ``"rs2"``)``,`` `` P ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``1e-8``, ``1e-5``)`` `` ``)`` `` `[`loadGwasTrack`](https://gladkia.github.io/igvShiny/reference/loadGwasTrack.md)`(`` `` ``session``, id ``=`` ``"igv"``, trackName ``=`` ``"gwas"``, tbl.gwas ``=`` ``gwas``,`` `` trackConfig ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` columns ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``chromosome ``=`` ``12``, position ``=`` ``13``, value ``=`` ``28``)`` `` ``)`` `` ``)`` ``#> removeTracksByName`` ``#> trackNames: gwas`` ``#> elementID: igv`` ``#> loadGwasTrack`` ``#> elementID: igv`` ``#> trackName: gwas`` ``#> gwasDataFilepath: tracks/file1ce4634ee282.gwas`` ``#> color: red`` ``#> trackHeight: 200`` ``#> autoscale: FALSE`` ``#> min: 0`` ``#> max: 35`` ``#> columns: 12,13,28`
...
`junctions.url`` ``<-`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(`` `` ``"https://raw.githubusercontent.com/igvteam/igv-data/main/data/test/"``,`` `` ``"splice_junctions/sampleA.SJ.out.bed.gz"`` `` ``)`` `` `[`loadSpliceJunctionTrackFromURL`](https://gladkia.github.io/igvShiny/reference/loadSpliceJunctionTrackFromURL.md)`(`` `` ``session``, id ``=`` ``"igv"``, trackName ``=`` ``"sampleA junctions"``,`` `` url ``=`` ``junctions.url``, indexURL ``=`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(``junctions.url``, ``".tbi"``)``,`` `` trackHeight ``=`` ``150``,`` `` trackConfig ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` colorBy ``=`` ``"motif"``,`` `` labelWith ``=`` ``"uniquelyMapped"``,`` `` minUniquelyMappedReads ``=`` ``5``,`` `` hideAnnotatedJunctions ``=`` ``FALSE`` `` ``)`` `` ``)`` ``#> removeTracksByName`` ``#> trackNames: sampleA junctions`` ``#> elementID: igv`` ``#> loadSpliceJunctionTrackFromURL`` ``#> elementID: igv`` ``#> trackName: sampleA junctions`` ``#> url: https://raw.githubusercontent.com/igvteam/igv-data/main/data/test/splice_junctions/sampleA.SJ.out.bed.gz`` ``#> indexURL: https://raw.githubusercontent.com/igvteam/igv-data/main/data/test/splice_junctions/sampleA.SJ.out.bed.gz.tbi`` ``#> trackHeight: 150`` ``#> displayMode: COLLAPSED`` ``#> colorBy: motif`` ``#> labelWith: uniquelyMapped`` ``#> minUniquelyMappedReads: 5`` ``#> hideAnnotatedJunctions: FALSE`
...
`gff3.url`` ``<-`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(`` `` ``"https://s3.amazonaws.com/igv.org.genomes/hg38/"``,`` `` ``"Homo_sapiens.GRCh38.94.chr.gff3.gz"`` `` ``)`` `` ``startupTrack`` ``<-`` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` name ``=`` ``"genes"``,`` `` type ``=`` ``"annotation"``,`` `` format ``=`` ``"gff3"``,`` `` url ``=`` ``gff3.url``,`` `` indexed ``=`` ``FALSE``,`` `` displayMode ``=`` ``"EXPANDED"`` `` ``)`` `` ``widget`` ``<-`` `[`igvShiny`](https://gladkia.github.io/igvShiny/reference/igvShiny.md)`(``genomeOptions``, tracks ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``startupTrack``)``)`` `[`str`](https://rdrr.io/r/utils/str.html)`(``widget``$``x``$``tracks``[[``1``]``]``)`` ``#> List of 6`` ``#> $ name : chr "genes"`` ``#> $ type : chr "annotation"`` ``#> $ format : chr "gff3"`` ``#> $ url : chr "https://s3.amazonaws.com/igv.org.genomes/hg38/Homo_sapiens.GRCh38.94.chr.gff3.gz"`` ``#> $ indexed : logi FALSE`` ``#> $ displayMode: chr "EXPANDED"`

In an app that goes inside
[`renderIgvShiny()`](https://gladkia.github.io/igvShiny/reference/renderIgvShiny.md):

`output``$``igv`` ``<-`` `[`renderIgvShiny`](https://gladkia.github.io/igvShiny/reference/renderIgvShiny.md)`(``{`` `` `[`igvShiny`](https://gladkia.github.io/igvShiny/reference/igvShiny.md)`(``genomeOptions``, tracks ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``startupTrack``)``)`` `` ``}``)`
