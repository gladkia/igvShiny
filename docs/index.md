...
\
`options`` ``<-`` `[`parseAndValidateGenomeSpec`](https://gladkia.github.io/igvShiny/reference/parseAndValidateGenomeSpec.md)`(``genomeName ``=`` ``"hg38"``, initialLocus ``=`` ``"NDUFS2"``)`\
\
...
`    `[`igvShiny`](https://gladkia.github.io/igvShiny/reference/igvShiny.md)`(``options``)`\
`  ``}``)`\
`}`\
\
