...
- Switch the hg19 and hg38 assets to igv.org, off a hgdownload host
  answering in tens of seconds
  ([\#167](https://github.com/gladkia/igvShiny/issues/167))
...
- Fix the demo BAM track, which drew GRCh37 reads against the hg38
  reference ([\#168](https://github.com/gladkia/igvShiny/issues/168))
...
- Rewrite the demo apps on bslib, replacing 15 legacy apps with 7
  single-topic ones
  ([\#64](https://github.com/gladkia/igvShiny/issues/64))
- Remove the forked Connect app, which now runs the package demo itself
  ([\#64](https://github.com/gladkia/igvShiny/issues/64))
- Extend the flagship demo with the splice junction and
  removeTracksByName calls
  ([\#64](https://github.com/gladkia/igvShiny/issues/64))
...
- Add loadCramTrackFromLocalData, serving a cram file and its index to
  igv.js ([\#102](https://github.com/gladkia/igvShiny/issues/102))
...
- Resolve tair10 through the igv.js registry, dropping the self-hosted
  fasta ([\#143](https://github.com/gladkia/igvShiny/issues/143))
- Read the rhos sequence and genes from the UCSC assembly hub instead of
  gladki.pl ([\#143](https://github.com/gladkia/igvShiny/issues/143))
...
- Remove the mm10 and danRer11 reference workaround, unneeded with
  igv.js 3.x ([\#107](https://github.com/gladkia/igvShiny/issues/107))
...
- Wrap the 25 over-long lines in `R/igvShiny.R` at 80 characters
  (BiocCheck)
- Move [`paste()`](https://rdrr.io/r/base/paste.html) out of
  [`warning()`](https://rdrr.io/r/base/warning.html) calls, keeping the
